...
class RutinaModificable extends Component {
  ...
  subir(dia, id_ejercicio, combinado) {
    ...
    var i = 0
    var flag = 0
    //la cantidad de ejercicios del mismo dia que encontro
    var fallador = 0
    while (i < this.state.rutina.length) {
      rutinaNueva.push(this.state.rutina[i])
      if (fallador != 0) {
        rutinaCopia.push(this.state.rutina[i])
      }
          ...
          if (fallador != 1) {
            ...
            if (combinado == null) {
              if (this.state.rutina[aux].combinado == null) {
                ...
                for (a = (i + 1); a < this.state.rutina.length; a++) {
                  rutinaNueva.push(this.state.rutina[a])
                }
              }
              ...
              if (rutinaCopia.length == 1) {
                rutinaNueva[aux] = this.state.rutina[i]
                rutinaNueva[(aux + 1)] = this.state.rutina[(i + 1)]
                rutinaNueva[i + 1] = this.state.rutina[aux]
                for (a = (i + 2); a < this.state.rutina.length; a++) {
                  rutinaNueva.push(this.state.rutina[a])
                }
              } else {
                rutinaNueva[aux] = this.state.rutina[i]
                ...
                for (a = 0; a < (rutinaCopia.length - 1); a++) {
                  rutinaNueva.push(rutinaCopia[a])
                }
                for (a = (i + 2); a < this.state.rutina.length; a++) {
                  rutinaNueva.push(this.state.rutina[a])
                }
    ...
    while (i < this.state.rutina.length) {
      ...
      if ((id_ejercicio == this.state.rutina[i].id_ejercicio && dia == this.state.rutina[i].dia) && flag != 1) {
        ...
        if (aux < this.state.rutina.length) {
          if (combinado == null) {
            console.log("holasfffffsfffffffffffffffffffffffffffffff")
                    ...
                    <View style={styles.cuadraditos}>
                      ...
                      >
                        <FlatList
                          ...
                          data={this.state.rutina.sort((a,b) => a.posicion.localeCompare(b.posicion))}
                          initialNumToRender={50}
                          keyExtractor={(item) => {
